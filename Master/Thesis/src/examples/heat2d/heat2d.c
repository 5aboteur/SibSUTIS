...
//      draw_heat(NXPROB,NYPROB);
	...
	const double time_taken = (((double)en.tv_sec) + (double)en.tv_usec * 1.0e-6) -
            (((double)st.tv_sec) + (double)st.tv_usec * 1.0e-6);
        const double msgs_recv = ((double)(STEPS * (numtasks - 1)));
        const double data_recv = (((double)STEPS) * ((double)sizeof(float)) *
            ((double)(numtasks - 1))) / (1024.0 * 1024.0);
