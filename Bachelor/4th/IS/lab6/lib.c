...
/*
unsigned long long hashy(long long n)
{
	char ch;
	char *nbuf;
	unsigned long long res = 0, p = 1;

	nbuf = malloc(sizeof(char) * sizeof(n));

	for (unsigned long long i = 0; i < sizeof(n); ++i)
		nbuf[i] = ((n >> (i * 8)) & 0XFF);

	for (unsigned long long i = 0; i < sizeof(n); ++i, p *= MAGIC) {
		ch = nbuf[i];
		res += ((int) ch * p);
	}
...
//			printf("r: %d, arr[r]: %c\n", r, arr[r]);
	...
	for (int i = 0; i < 3; ++i) arr[i] = s_arr[i];
